...
class UniformCostSearch {
    ...
    public int uniformCostSearch(int[][] adjacencyMatrix, int asal, int tujuan) {
        ...
        for (int asalvertex = 1; asalvertex <= numberOfNodes; asalvertex++) {
            if (numberOfNodes >= 0)
                System.arraycopy(adjacencyMatrix[asalvertex], 1, this.adjacencyMatrix[asalvertex], 1, numberOfNodes);
        }
    ...
    public void cetakJalur() {
        for (int i = jalur.size() - 1; i >= 0; i--) {
            if (i == 0)
                System.out.println(find_route.city.get(jalur.get(i) - 1));
            else
                System.out.print(find_route.city.get(jalur.get(i) - 1) + " -> ");
        }
    }
}
